  ...
  return (
    <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 mb-3 animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex gap-4">
        <div className="w-20 h-20 bg-slate-50 rounded-2xl flex-shrink-0 overflow-hidden border border-slate-100/50 flex items-center justify-center relative">
          {item.menuItemImageUrl ? (
            <img src={item.menuItemImageUrl} alt={item.menuItemName} className="w-full h-full object-cover" />
          ) : (
